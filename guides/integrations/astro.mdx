...
Next, define your content collection. This tells Astro how to fetch and validate your data from Marble. Create a file named `src/config.ts` and add the following code.

This configuration defines a `posts` collection, and includes a loader function that fetches your posts from the Marble API.

<Note>
  For complete TypeScript type definitions, see our [Types reference](/api/types). The schema below matches the `Post` type from
  the API.
</Note>
