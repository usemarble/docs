---
title: Introduction to Marble CMS
description: Marble is a simple, headless CMS built for writers and developers. Manage and publish content with a clean editor, flexible REST API, and team collaboration features.
---

**Marble is a simple, headless CMS built for writers and developers.** It helps you manage and publish content without the complexity of traditional CMS platforms.

## Why use Marble?

<CardGroup cols={2}>
  <Card title="Clean Editor" icon="pen">
    A distraction-free writing experience with Markdown and AI assistance.
  </Card>
  <Card title="Headless API" icon="code">
    Pull your content into any frontend (Next.js, Astro, etc.) with a flexible
    REST API.
  </Card>
  <Card title="Team Collaboration" icon="users">
    Invite others to write, edit, and review posts in a shared workspace.
  </Card>
  <Card title="Image Support" icon="image">
    Upload images directly to Marble or link from your own storage solution. All media is delivered through Cloudflare's globally distributed CDN for fast loading worldwide.
  </Card>
</CardGroup>

## Who is it for?

- **Developers** who want to build custom sites without a bloated, monolithic CMS.
- **Writers** who just want a clean and simple interface to write and publish.
- **Small teams** that need a fast, collaborative way to manage content.

<Info>
  **It's early days.** We're shipping fast and improving based on feedback from
  users like you.
</Info>

## Ready to get started?

<Card title="Quickstart Guide" href="/guides/quickstart" icon="rocket">
  Follow our quickstart guide to set up your account and create your first post
  in minutes.
</Card>
