...
<Warning>
  **Important:** Never expose your `MARBLE_WORKSPACE_KEY` in client-side code.
  ...
  [webhooks](/content/guides/webhooks) to trigger rebuilds.
</Warning>
...
This configuration defines a `posts` collection, specifies a schema for your post data using Zod for validation, and includes a loader function that fetches all your posts from the Marble API.

<Note>
For complete TypeScript type definitions, see our [TypeScript Types reference](/content/api-reference/types). The schema below matches the `Post` type from the API.
</Note>

```typescript src/content.config.ts
import { z, defineCollection } from "astro:content";
import type { MarblePostList } from "./types/marble"; // Optional: import types for better IDE support
